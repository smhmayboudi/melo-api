...
FROM node:12.18.1-alpine AS build_node_melo-api
ARG NODE_ENV=production
ENV NODE_ENV=${NODE_ENV}
WORKDIR /app
COPY --from=build_node_modules /build/node_modules ./node_modules
COPY --from=build_node_app /build/dist/apps/melo-api/main.js ./dist/apps/melo-api/main.js
CMD ["node","./dist/apps/melo-api/main.js"]
