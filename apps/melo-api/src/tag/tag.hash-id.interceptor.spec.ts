...
describe("TagHashIdInterceptor", () => {
  ...
  it("should be defined", () => {
    expect(new TagHashIdInterceptor(service)).toBeDefined();
  });

  it("intercept should be called undefined", () => {
    const callHandler: CallHandler = {
      handle: jest.fn(() => of(undefined)),
    };
    new TagHashIdInterceptor(service)
  ...
  it("intercept should be called with data: tag", () => {
    const callHandler: CallHandler = {
      handle: jest.fn(() => of(tag)),
    };
    new TagHashIdInterceptor(service)
      .intercept(executionContext, callHandler)
      .subscribe();
  ...
  it("intercept should be called with data: array of tag", () => {
    const callHandler: CallHandler = {
      handle: jest.fn(() => of([tag])),
    };
    new TagHashIdInterceptor(service)
